 ...
 * @class RdfaEditorSchepenenAanstellingPlugin
 * @constructor
 * @extends EmberService
 */
const RdfaEditorSchepenenAanstellingPlugin = Service.extend({
  insertSchepenenAanstellingText: 'http://mu.semte.ch/vocabularies/ext/schepenenAanstellingText',
  schepenenAanstellingTable: 'http://mu.semte.ch/vocabularies/ext/schepenenAanstellingTable',
  ...
  execute: task(function * (hrId, contexts, hintsRegistry, editor) {
    ...
    for(let context of contexts){
      ...
      if(triple.predicate == this.insertSchepenenAanstellingText){
      ...
      if(triple.predicate == this.schepenenAanstellingTable && !hints.find(h => h.location[0] == domNodeRegion[0] && h.location[1] == domNodeRegion[1])){
  ...
  detectRelevantContext(context){
    if(context.context.slice(-1)[0].predicate == this.insertSchepenenAanstellingText){
      return context.context.slice(-1)[0];
    }
    if(context.context.slice(-1)[0].predicate == this.schepenenAanstellingTable){
      return context.context.slice(-1)[0];
    }
    return null;
  },
  ...
  generateHintsForContext(context, instructiveTriple, domNode, editor, options = {}){
    ...
    if(instructiveTriple.predicate == this.schepenenAanstellingTable){
  ...
  findDomNodeForContext(editor, context, condition){
    ...
    let domNode = richNodes
          ...
          .find(d => d);
    if(!domNode){
      warn(`Trying to work on unattached domNode. Sorry can't handle these...`, {id: 'schepenenAanstelling.domNode'});
    }
    return domNode;
  },
...
RdfaEditorSchepenenAanstellingPlugin.reopen({
  who: 'editor-plugins/schepenen-aanstelling-card'
});
export default RdfaEditorSchepenenAanstellingPlugin;
